...
func FuzzInitialAvatar(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, name string) {
		svg := avatar.GenerateInitial(name)
...
func FuzzInitialAvatarXSS(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, input string) {
		svg := avatar.GenerateInitial(input)
...
func FuzzGravatarHashConsistency(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, email string) {
		url1 := avatar.GravatarURL(email)
		url2 := avatar.GravatarURL(strings.ToLower(strings.TrimSpace(email)))
...
func FuzzDeterministicIdenticon(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, id int64) {
		png1, _ := avatar.Generate(id)
		png2, _ := avatar.Generate(id)
