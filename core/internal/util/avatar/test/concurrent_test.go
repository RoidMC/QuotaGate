...
func TestVisualOutput(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			pngData, err := avatar.Generate(tc.id)
			if err != nil {
			...
			svgData := avatar.GenerateSVG(tc.id)
	...
	for _, tc := range nameCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			svgData := avatar.GenerateInitial(tc.input)
	...
	for _, tc := range emailCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			url := avatar.GravatarURL(tc.email)
