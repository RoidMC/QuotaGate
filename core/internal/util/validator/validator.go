...
func (v *Validator) Validate(input string) error {
	if input == "" {
		...
		return fmt.Errorf("quotagate/validator: input is empty")
	}

	length := len([]rune(input))
	if v.MinLength > 0 && length < v.MinLength {
		return fmt.Errorf("quotagate/validator: input length %d is less than minimum %d", length, v.MinLength)
	}
	if v.MaxLength > 0 && length > v.MaxLength {
		return fmt.Errorf("quotagate/validator: input length %d exceeds maximum %d", length, v.MaxLength)
	}

	if v.Regex != nil && !v.Regex.MatchString(input) {
		return fmt.Errorf("quotagate/validator: input does not match required pattern")
	}

	if v.SpecialChars != "" {
		for _, c := range input {
			if !strings.ContainsRune(v.SpecialChars, c) && !unicode.IsLetter(c) && !unicode.IsDigit(c) {
				return fmt.Errorf("quotagate/validator: input contains invalid character: %q", c)
			}
		}
	}
...
func (ev *EmailValidator) Validate(email string) error {
	if !ev.Regex.MatchString(email) {
		return fmt.Errorf("quotagate/validator: invalid email format")
	}
	...
	if len(ev.Whitelist) > 0 {
		...
		if !allowed {
			return fmt.Errorf("quotagate/validator: email domain %s is not in whitelist", domain)
		}
	}
	...
	for _, d := range ev.Blacklist {
		if strings.EqualFold(domain, d) {
			return fmt.Errorf("quotagate/validator: email domain %s is in blacklist", domain)
		}
	}
...
// NewUsernameValidator creates a username validator.
...
func (uv *UsernameValidator) Validate(username string) error {
	if !uv.AllowEmpty && username == "" {
		return fmt.Errorf("quotagate/validator: username is empty")
	}

	length := len([]rune(username))
	if uv.MinLength > 0 && length < uv.MinLength {
		return fmt.Errorf("quotagate/validator: username length %d is less than minimum %d", length, uv.MinLength)
	}
	if uv.MaxLength > 0 && length > uv.MaxLength {
		return fmt.Errorf("quotagate/validator: username length %d exceeds maximum %d", length, uv.MaxLength)
	}

	if uv.Regex != nil && !uv.Regex.MatchString(username) {
		return fmt.Errorf("quotagate/validator: username contains invalid characters")
	}

	for _, reserved := range uv.ReservedNames {
		if strings.EqualFold(username, reserved) {
			return fmt.Errorf("quotagate/validator: username %s is reserved", username)
		}
	}
...
func (pv *PasswordValidator) Validate(password string) error {
	if !pv.AllowEmpty && password == "" {
		return fmt.Errorf("quotagate/validator: password is empty")
	}

	length := len([]rune(password))
	if pv.MinLength > 0 && length < pv.MinLength {
		return fmt.Errorf("quotagate/validator: password length %d is less than minimum %d", length, pv.MinLength)
	}
	if pv.MaxLength > 0 && length > pv.MaxLength {
		return fmt.Errorf("quotagate/validator: password length %d exceeds maximum %d", length, pv.MaxLength)
	}
	...
	for _, c := range password {
		if unicode.IsUpper(c) {
			...
			return fmt.Errorf("quotagate/validator: password contains invalid character: %q", c)
		}
	}

	if upperCount < pv.MinUppercase {
		return fmt.Errorf("quotagate/validator: password must contain at least %d uppercase letter(s)", pv.MinUppercase)
	}
	if lowerCount < pv.MinLowercase {
		return fmt.Errorf("quotagate/validator: password must contain at least %d lowercase letter(s)", pv.MinLowercase)
	}
	if digitCount < pv.MinDigits {
		return fmt.Errorf("quotagate/validator: password must contain at least %d digit(s)", pv.MinDigits)
	}
	if specialCount < pv.MinSpecialChars {
		return fmt.Errorf("quotagate/validator: password must contain at least %d special character(s)", pv.MinSpecialChars)
	}
...
func ValidateWithRegex(input string, pattern string) error {
	...
	if err != nil {
		return fmt.Errorf("quotagate/validator: invalid regex pattern: %w", err)
	}
	if !re.MatchString(input) {
		return fmt.Errorf("quotagate/validator: input does not match pattern: %s", pattern)
	}
	return nil
}
