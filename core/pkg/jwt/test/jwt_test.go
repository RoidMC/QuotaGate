...
func TestTokenUniqueness(t *testing.T) {
	m := mustNewManager(t, kexjwt.ModeJWE, testSignKey, testEncKey, 15*time.Minute, 7*24*time.Hour)
	t1, _ := m.GenerateAccessToken("user-123", "admin", "", []string{"admin"})
	t2, _ := m.GenerateAccessToken("user-123", "admin", "", []string{"admin"})
	if t1 == t2 {
...
func TestSM2SM4TokenUniqueness(t *testing.T) {
	...
	t1, _ := m.GenerateAccessToken("user-guomi", "admin", "", []string{"admin"})
	t2, _ := m.GenerateAccessToken("user-guomi", "admin", "", []string{"admin"})
	if t1 == t2 {
		t.Error("two tokens should be different")
	}
}
...
func TestSM2LegacyAliasSM2(t *testing.T) {
	...
	token, err := m.GenerateAccessToken("user-alias", "admin", "", []string{"admin"})
	if err != nil {
		t.Fatalf("GenerateAccessToken: %v", err)
	}
	claims, err := m.ParseToken(token)
...
func TestSM2LegacyAliasSM2SM3(t *testing.T) {
	...
	token, err := m.GenerateAccessToken("user-sm2sm3", "admin", "", []string{"admin"})
	if err != nil {
		t.Fatalf("GenerateAccessToken: %v", err)
	}
	claims, err := m.ParseToken(token)
...
func TestSM4LegacyAliasSM4GCM(t *testing.T) {
	...
	token, err := m.GenerateAccessToken("user-sm4legacy", "admin", "", []string{"admin"})
	if err != nil {
		t.Fatalf("GenerateAccessToken: %v", err)
	}
	claims, err := m.ParseToken(token)
