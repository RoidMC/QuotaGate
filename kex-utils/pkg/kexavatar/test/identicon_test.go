...
func TestGenerate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			data, err := kexavatar.Generate(tt.id)
			if err != nil {
			...
			if img.Bounds().Dx() != kexavatar.DefaultSize {
				t.Errorf("Image width = %d, want %d", img.Bounds().Dx(), kexavatar.DefaultSize)
			}
			if img.Bounds().Dy() != kexavatar.DefaultSize {
				t.Errorf("Image height = %d, want %d", img.Bounds().Dy(), kexavatar.DefaultSize)
			}
		})
	}
}
...
func TestDeterministic(t *testing.T) {
	...
	data1, err := kexavatar.Generate(id)
	if err != nil {
		t.Fatalf("Generate error: %v", err)
	}

	data2, err := kexavatar.Generate(id)
	if err != nil {
		t.Fatalf("Generate error: %v", err)
	}
...
func TestDifferentIds(t *testing.T) {
	data1, err := kexavatar.Generate(1)
	if err != nil {
		t.Fatalf("Generate error: %v", err)
	}

	data2, err := kexavatar.Generate(2)
	if err != nil {
		t.Fatalf("Generate error: %v", err)
	}
...
func TestGenerateSVGBase64(t *testing.T) {
	id := int64(12345)
	result := kexavatar.GenerateSVGBase64(id)
...
func TestWithOptions(t *testing.T) {
	...
	identicon := kexavatar.NewIdenticon(
		kexavatar.WithSize(customSize),
		kexavatar.WithGrid(customGrid),
	)
...
func BenchmarkGenerate(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = kexavatar.Generate(int64(i))
	}
}

func BenchmarkGenerateSVG(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_ = kexavatar.GenerateSVG(int64(i))
	}
}
