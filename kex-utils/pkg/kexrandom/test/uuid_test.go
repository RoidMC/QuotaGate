...
func TestNewUUIDTimeOrdered(t *testing.T) {
	prev, err := kexrandom.NewUUID()
	if err != nil {
		t.Fatalf("NewUUID failed: %v", err)
	}

	for i := 0; i < 100; i++ {
		curr, err := kexrandom.NewUUID()
		if err != nil {
			t.Fatalf("NewUUID failed: %v", err)
		}
...
func TestFormatUUID(t *testing.T) {
	...
	formatted := kexrandom.FormatUUID(uuid)
...
func TestParseUUID(t *testing.T) {
	...
	uuid, err := kexrandom.ParseUUID(original)
	if err != nil {
		t.Fatalf("ParseUUID failed: %v", err)
	}

	result := kexrandom.FormatUUID(uuid)
	if result != original {
...
func TestParseUUIDWithoutDashes(t *testing.T) {
	...
	uuid, err := kexrandom.ParseUUID(s)
	if err != nil {
		t.Fatalf("ParseUUID failed: %v", err)
	}
...
func TestParseUUIDInvalid(t *testing.T) {
	_, err := kexrandom.ParseUUID("not-a-uuid")
	if err == nil {
		t.Error("expected error for invalid UUID")
	}

	_, err = kexrandom.ParseUUID("01234567-89ab-7cde-8012-3456789abcdeff")
	if err == nil {
...
func TestUUIDRoundTrip(t *testing.T) {
	for i := 0; i < 100; i++ {
		original, err := kexrandom.NewUUID()
		if err != nil {
			t.Fatalf("NewUUID failed: %v", err)
		}

		formatted := kexrandom.FormatUUID(original)
		parsed, err := kexrandom.ParseUUID(formatted)
		if err != nil {
			t.Fatalf("ParseUUID failed: %v", err)
		}
